...
export async function onRequest({ request, env }) {
  ...
  if (!d) d = { dates:[...seed], updatedAt:new Date().toISOString() };
  const ids = new Set((d.dates || []).map(x => x.id));
  let changed = false;
  for (const s of seed) if (!ids.has(s.id)) { d.dates.push(s); changed = true; }
  if (changed) { d.updatedAt = new Date().toISOString(); await putJson(env.WUZZE_KV, key, d); }
  if (request.method === "GET") { if (!changed) await putJson(env.WUZZE_KV, key, d); return json(d); }
  ...
  if (request.method === "POST") {
    ...
    d.dates.push({
      ...
      title:String(b.title).trim().slice(0,160), date:String(b.date).slice(0,10), time:String(b.time).slice(0,5),
      owner:String(b.owner || "").trim().slice(0,120), location:String(b.location || "").trim().slice(0,160),
