...
export async function onRequest({ request, env }) {
  ...
  const d = await getJson(env.WUZZE_KV, key, { tasks:[], updatedAt:null });
  ...
  if (request.method === "POST") {
    ...
    d.tasks.unshift({
      ...
      createdBy:String(b.createdBy || "Team").trim().slice(0,80),
    ...
    t.status=b.status; t.updatedBy=String(b.by || "Team").slice(0,80); t.updatedAt=new Date().toISOString();
